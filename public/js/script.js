  ...
  $(document).ready(function () {
    ...
    function sendMail() {
      $(".sendMail").on("click", async function (e) {
        ...
        // Validação
        if (!name) return showAlert("error", "Por favor, preencha o nome.");
        if (!email) return showAlert("error", "Por favor, preencha o e-mail.");

        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email))
          return showAlert("error", "E-mail inválido.");

        if (!subject)
          return showAlert("error", "Por favor, preencha o assunto.");
        if (!message)
        ...
        try {
          ...
          if (data.status === "success") {
            ...
            showAlert("error", data.message || "Erro ao enviar a mensagem.");
          }
        } catch (error) {
          showAlert(
            "error",
